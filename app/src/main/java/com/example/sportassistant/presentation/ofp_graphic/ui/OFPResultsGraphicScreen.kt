            ...
            DropdownMenu (
                ...
                stringArrayResource(R.array.date_periods).forEachIndexed { index, option ->
                    DropdownMenuItem(
                        ...
                        onClick = {
                            calculatePeriod(ofpResultsGraphicViewModel, index)
                            ofpResultsGraphicViewModel.setDatePeriod(option)
                            expandedDate = false
                        },
                        contentPadding = ExposedDropdownMenuDefaults.ItemContentPadding,
                        modifier = Modifier.fillMaxWidth()
                        ...
                        TextButton(onClick = {
                            if (
                                ...
                                val startDate = dateRangePickerState.selectedStartDateMillis?.let { it ->
                                    ...
                                    ofpResultsGraphicViewModel.setStartDate(date)
                                    date.format(formatter)
                                } ?: ""

                                val endDate = dateRangePickerState.selectedEndDateMillis?.let {
                                    val date = LocalDate.ofInstant(Instant.ofEpochMilli(it), ZoneId.systemDefault())
                                    ofpResultsGraphicViewModel.setEndDate(date)
                                    date.format(formatter)
                                } ?: ""
                                ofpResultsGraphicViewModel.setDatePeriod("$startDate - $endDate")
                                showDialog = false
                                expandedDate = false
                                missingDate = false
                            } else {
                    ...
                    is ApiResponse.Success -> {
                        ...
                        categories.forEach { option ->
                            DropdownMenuItem(
                                ...
                                onClick = {
                                    ofpResultsGraphicViewModel.setCategory(option.name)
                                    ofpResultsGraphicViewModel.setSelectedCategory(option.id)
...
private fun isModified(prevState: OFPResultsGraphicUiState, uiState: OFPResultsGraphicUiState): Boolean {
...
private fun isAllFilled(state: OFPResultsGraphicUiState): Boolean {
    return (state.endDate != null && state.startDate != null && state.selectedCategoryId != null)
}

private fun calculatePeriod(viewModel: OFPResultsGraphicViewModel, option: Int) {
