...
//                top = 15.dp,
            ...
            DropdownMenu (
                ...
                stringArrayResource(R.array.date_periods).forEachIndexed { index, option ->
                    DropdownMenuItem(
                        ...
                        onClick = {
                            calculatePeriod(anthropometricParamsGraphicViewModel, index)
                            anthropometricParamsGraphicViewModel.setDatePeriod(option)
                            expandedDate = false
                        },
                        contentPadding = ExposedDropdownMenuDefaults.ItemContentPadding,
                        modifier = Modifier.fillMaxWidth()
                        ...
                        TextButton(onClick = {
                            if (
                                ...
                                val startDate = dateRangePickerState.selectedStartDateMillis?.let { it ->
                                    ...
                                    anthropometricParamsGraphicViewModel.setStartDate(date)
                                    date.format(formatter)
                                } ?: ""

                                val endDate = dateRangePickerState.selectedEndDateMillis?.let {
                                    val date = LocalDate.ofInstant(Instant.ofEpochMilli(it), ZoneId.systemDefault())
                                    anthropometricParamsGraphicViewModel.setEndDate(date)
                                    date.format(formatter)
                                } ?: ""
                                anthropometricParamsGraphicViewModel.setDatePeriod("$startDate - $endDate")
                                showDialog = false
                                expandedDate = false
                                missingDate = false
                            } else {
        ...
        ExposedDropdownMenuBox(
            ...
            onExpandedChange = { expandedMeasure = it },
            modifier = Modifier.fillMaxWidth().padding(
                start = screenSizeProvider.getEdgeSpacing(),
                end = screenSizeProvider.getEdgeSpacing(),
                top = 15.dp,
            ...
            StyledCardTextField(
                value = uiState.measureString.ifEmpty { stringResource(R.string.ant_params_measure) },
                onValueChange = { },
                enabled = false,
                modifier = Modifier.menuAnchor().padding(
                    start = 15.dp,
                ...
                readOnly = true,
                trailingIcon = {
                    GetDropdownTrailingIcon(expandedMeasure)
                },
            )
            DropdownMenu (
                expanded = expandedMeasure,
                ...
                stringArrayResource(R.array.ant_params_measures).forEachIndexed { index, option ->
                    DropdownMenuItem(
                        text = {
                            Text(
                                text = option,
                        ...
                        onClick = {
                            anthropometricParamsGraphicViewModel.setMeasureString(option)
                            getMeasure(anthropometricParamsGraphicViewModel, index)
                            expandedMeasure = false
                        },
                        contentPadding = ExposedDropdownMenuDefaults.ItemContentPadding,
                        modifier = Modifier.fillMaxWidth()
                    )
...
private fun isAllFilled(state: AnthropometricParamsGraphicUiState): Boolean {
    return (state.endDate != null && state.startDate != null && state.measure != null)
}

private fun calculatePeriod(viewModel: AnthropometricParamsGraphicViewModel, option: Int) {
...
private fun getMeasure(viewModel: AnthropometricParamsGraphicViewModel, option: Int) {
    val measure = when (option) {
        0 -> {
            AnthropometricParamsMeasures.weight
        }
        1 -> {
            AnthropometricParamsMeasures.height
        }
        2 -> {
            AnthropometricParamsMeasures.chestCircumference
        }
        else -> {
            null
        }
    }
    viewModel.setMeasure(measure)
}
